...
class ProductUtil extends Util
{
    ...
    public function getVariationGroupPrice($variation_id, $price_group_id, $tax_id)
    {
        ...
        if (!empty($price_group) && isset($price_group->price_type)) {
            if ($price_group->price_type == 'percentage') {
                // Ensure variation exists before accessing properties
                $variation = Variation::find($variation_id);
                if ($variation) {
                    $price_inc_tax = $this->calc_percentage($variation->sell_price_inc_tax, $price_group->price_inc_tax);
                } else {
                    $price_inc_tax = 0; // Set a default value if variation is not found
                }
            } else {
                $price_inc_tax = $price_group->price_inc_tax;
            }
        } else {
            // Use a fallback price if $price_group is NULL
            $variation = Variation::find($variation_id);
